//@see https://github.com/adwellj/node-tfjs-retrain

//@ts-check
const tf = (process.env.gpu === true || process.env.gpu === "true") ? require("@tensorflow/tfjs-node-gpu") : require("@tensorflow/tfjs-node");
...
async function loadDecapitatedMobilenet() {
    try{
        const mobilenet = await tf.loadLayersModel(
            `file://${path.resolve("../data/mobilenet/model.json")}`
    ...
    buildRetrainingModel(denseUnits, numClasses, learningRate) {
        ...
        this.model = tf.sequential({
            layers: [
                ...
                // parameters).
                tf.layers.flatten({
                    inputShape: [7,7,1024] //this.decapitatedMobilenet.outputs[0].shape.slice(1)
                }),
                // Layer 1.
                tf.layers.dense({
                    units: denseUnits,
